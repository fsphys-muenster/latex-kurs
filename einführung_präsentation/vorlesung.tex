...
\begin{frame}[<+->][fragile]
	...
	\begin{itemize}
		...
		$\rightarrow$~problematische Stellen finden
...
\begin{frame}[<+->][fragile]
	\begin{itemize}
		...
		\item \lstinline!\input{datei.tex}!: Die Datei wird exakt an der Stelle in das Dokument eingefügt $\rightarrow$\ Übersichtlichkeit im Quelltext!
	\end{itemize}
\end{frame}
...
\begin{frame}[<+->][fragile]
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \lstinline!\begin{equation} ... \end{equation}!: \emph{Einzelne} nummerierte, abgesetzte Gleichung
			\item \lstinline!\begin{align} ... \end{align}!: \emph{Mehrere} nummerierte, abgesetzte, \emph{ausgerichtete} Gleichungen
		\end{itemize}
		...
		\item \strong{Achtung:} Absätze (Leerzeilen) sind in Mathe-Umgebungen verboten!
	\end{itemize}
\end{frame}
...
\begin{frame}[<+->][fragile]
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Klammern setzen!\footnote<.->{\lstinline!^! und \lstinline!_! sind eigentlich Befehle; ohne Klammern wird das nächste Zeichen als Parameter verwendet.} \lstinline!U_{ind}! $\rightarrow$~$U_{ind}$
			\item Noch besser: \lstinline!U_\text{ind}! $\rightarrow$~$U_\text{\rmfamily ind}$
		\end{itemize}
...
\begin{frame}[<+->][fragile]
	\begin{itemize}
		...
		\begin{itemize}
			\item Anfangsbuchstabe klein: kleiner Buchstabe; \lstinline!\gamma! $\rightarrow$\ $\gamma$
			\item Anfangsbuchstabe groß: großer Buchstabe; \lstinline!\Gamma! $\rightarrow$\ $\Gamma$
		\end{itemize}
		\item Warum gibt es kein \lstinline!\Alpha!?
		\item \lstinline!\sum_{n = 0}^{\infty}!
		$\rightarrow$~$\displaystyle\sum_{n=0}^{\infty}$, \lstinline!\int_{-\infty}^{\infty}!
		$\rightarrow$~$\displaystyle\int_{-\infty}^{\infty}$
...
\begin{frame}[<+->][fragile]
	...
	\begin{itemize}
		...
		\lstinline!\sin x! $\rightarrow$~$\sin x$
		
		\lstinline!sin x! $\rightarrow$~$sin x = s \cdot i \cdot n \cdot x$ (falsch!)
		...
		\lstinline!\int f(x) \, \mathrm{d}x! $\rightarrow$\ $\displaystyle\int f(x) \, \mathrm{d}x$
		
		\lstinline!\frac{\mathrm{d}f}{\mathrm{d}x}! $\rightarrow$~$\displaystyle\frac{\mathrm{d}f}{\mathrm{d}x}$
...
\begin{frame}[<+->][fragile]
	...
	\begin{itemize}
		...
		\item "\lstinline!\cref{mond} zeigt den Mond!"
		$\rightarrow$~"{\rmfamily\cref{mond} zeigt den Mond}"
...
\begin{frame}[<+->][fragile]
	...
	\begin{itemize}
		...
		\lstinline!\cref{eq:einstein,eq:planck}! $\rightarrow$\ {\rmfamily\cref{eq:einstein,eq:planck}}
	\end{itemize}
\end{frame}
...
\begin{frame}[<+->][fragile]
	...
	\begin{itemize}
		...
		\item \lstinline!\includegraphics[scale=0.2, angle=140]{fig2.pdf}! $\rightarrow$\ \rlap{\includegraphics[scale=0.2, angle=140]{res/fig2.pdf}}
	\end{itemize}
\end{frame}
...
\begin{frame}[<+->][fragile]
	...
	\begin{itemize}
		...
		$\rightarrow$~Unterordner für Bilder etc.\ ist zu empfehlen; Übersichtlichkeit!
	\end{itemize}
\end{frame}
...
\begin{frame}[<+->][fragile]
	...
	\bigskip
	\visible<+->{\emph{Wichtig:} \lstinline!subfigure! bzw.\ \lstinline!subtable! verhält sich wie ein in den Fließtext eingefügtes Bild
	$\rightarrow$~auf Leerzeilen (=~Absätze) achten}
\end{frame}
...
\begin{frame}[<+->]
	...
	\begin{description}
		...
		\item["Cheat Sheet"] \url{https://ctan.org/pkg/latex-refsheet} (englisch)
...
\begin{frame}[<+->]
	...
	\begin{itemize}
		...
		\item Textbasiert $\rightarrow$~Befehlseingabe über die Kommandozeile
		\item Ausgabe der Diagramme in vielen Formaten, z.\,B.\ PDF
		
		$\rightarrow$~Einbinden in \LaTeX-Dokumente
...
\begin{frame}[<+->][fragile]
	...
	\begin{itemize}
		...
		$\rightarrow$~Zur Sicherheit immer Kommazahlen verwenden: \lstinline|1./2.|~=~\lstinline|0.5|}
